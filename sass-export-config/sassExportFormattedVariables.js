...
const sassExportCreatePropertyFormatter = require('./sassExportCreatePropertyFormatter');
const sortByReference = require('style-dictionary/lib/common/formatHelpers/sortByReference');
 ...
 * ```
 */
function sassExportFormattedVariables({ format, dictionary, outputReferences = false, formatting = {}, themeable = false, forcePrefix, transform = null}) {
  ...
  return allTokens
    .map(sassExportCreatePropertyFormatter({ outputReferences, dictionary, format, formatting, themeable, forcePrefix, transform }))
    .filter(function(strVal) { return !!strVal })
    .join(lineSeparator);
}

module.exports = sassExportFormattedVariables;
